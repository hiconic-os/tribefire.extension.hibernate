...
public class EnumCollectionExample_Main {
	...
	private final boolean WRITE_DATA = true;
	...
	private void run() throws Exception {
		...
		hsfb.setAdditionalProperties(asMap("hibernate.id.db_structure_naming_strategy", "single"));
		hsfb.postConstruct();
	...
	private void applyMappingXml() {
		EntityMapping entityMapping = EntityMapping.T.create();
		entityMapping.setXml(xml);

		ModelMetaDataEditor mdEditor = BasicModelMetaDataEditor.create(modelSupplier.get()).done();
		mdEditor.onEntityType(EnumCollectionEntity.T).addMetaData(entityMapping);
	}
